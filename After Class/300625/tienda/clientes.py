...
def eliminar_cliente_id_sql():
    try:
        ...
        # pedir id
        id_cliente = input("Ingrese ID del cliente a eliminar: ").strip()
        ...
        if cursor.rowcount == 0: #verifica la cantidad de filas afectadas
            print(f"No se encontró el cliente con el id {id_cliente}")
        else:
            conexion.commit()
            print(f"Cliente con {id_cliente} eliminado con éxito.")
    except sqlite3.Error as e:
        print(Fore.RED+f"Error al eliminar el cliente {e}")
    finally:
        conexion.close()
